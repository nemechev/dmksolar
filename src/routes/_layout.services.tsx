...
import { Ruler, Package, Plug, Wrench, Home as HomeIcon, Factory } from "lucide-react";
...
export const Route = createFileRoute("/_layout/services")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Проєктування, постачання, монтаж, підключення та сервіс сонячних електростанцій.",
      },
    ],
  }),
});
    ...
    <>
      ...
      <Section title={t("home.for.title")} className="bg-muted/40">
        <div className="grid gap-6 md:grid-cols-2">
          <Link
            to="/services/home"
            className="p-8 bg-background rounded-xl border border-border hover:border-primary transition"
          >
            ...
            <span className="mt-4 inline-block text-primary text-sm font-medium">
              {t("cta.more")} →
            </span>
          </Link>
          <Link
